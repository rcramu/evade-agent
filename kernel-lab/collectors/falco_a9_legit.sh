#!/bin/sh
# A9 product: workspace-only read. No /lab/secrets. Temporal inversion is a join.
set -eu
mkdir -p /workspace/documents /out
printf 'architecture note\n' >/workspace/documents/note.md
printf '#!/bin/sh\ncat -- "$1" >/dev/null\n' >/tmp/document-reader
chmod +x /tmp/document-reader
...
pid=$!
sleep 3
/tmp/document-reader /workspace/documents/note.md >/dev/null
sleep 3
